---
name: surface-language
description: Create, edit, review, format, and validate Surface application specifications written as KDL 2 files. Use for files containing a surface-lang node, requests to author a Surface specification, or work on the Surface language repository and its rung-1 Hello World application.
---
...
1. Confirm the document contains `surface-lang "0.1"`. Do not treat an arbitrary
...
7. Report requests outside the current language scope instead of inventing
   syntax.
...
- `surface-lang "0.1"` is the first semantic node and occurs once.
