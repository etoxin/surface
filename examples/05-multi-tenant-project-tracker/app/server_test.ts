...
Deno.test("the shipped UI includes role gates and stale tenant-response defenses", async () => {
  ...
  assertMatch(shippedHtml, /\/assets\/pico\.min\.css/);
  assertMatch(shippedHtml, /<main class="container">/);
  ...
  const handler = createRequestHandler(createDevelopmentState(), shippedHtml);
  const stylesheet = await handler(request("/assets/pico.min.css"));
  strictEqual(stylesheet.status, 200);
  strictEqual(stylesheet.headers.get("content-type"), "text/css; charset=utf-8");
  assert((await stylesheet.text()).length > 80_000);
});
